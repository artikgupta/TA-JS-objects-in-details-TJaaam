...
class Quiz {
  constructor(allQuestions = quizData, activeIndex = 0, score = 0) {
  ...
  nextQuestion() {
    if (this.activeIndex < this.allQuestions.length) {
      this.activeIndex += 1; // if active index is 0 then if we are +=1 then the 1 will be added to active index and assigned to the active index that means it would have become 1(0+1=1)
      this.createUI();
    }
    return;
  }
  ...
  createUI() {
    let { question, answers, correctIndex } = this.allQuestions[
      this.activeIndex
    ];
    let questions = new Question(question, answers, correctIndex);
    questions.createUI();
    ...
    document.querySelector(".question").addEventListener(
      ...
      (event) => {
        if (event.target.tagName == "SPAN") {
          ...
          let isAnswerCorrect = questions.isCorrect(selectedAnswer);
          if (isAnswerCorrect) {
...
class Question {
  ...
  isCorrect(answer) {
    return this.options.indexOf(answer) === this.correctAnswer;
  }
